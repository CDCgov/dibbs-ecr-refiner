...
def process_refiner(
    ...
    for jurisdiction_group in reportability_result["reportable_conditions"]:
        ...
        if jurisdiction_code not in metadata:
            ...
            for rsg_code in jurisdiction_group.conditions:
                rsg_code = rsg_code.code
                ...
                if not config_version_to_use:
                    # we shouldn't ever be here since the active guard should
                    # have been caught at the RSG lookup level, but check just in case
                    metadata[jurisdiction_code][rsg_code] = False
