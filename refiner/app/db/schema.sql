...
CREATE INDEX IF NOT EXISTS idx_conditions_child_snomed_codes ON conditions USING GIN (child_rsg_snomed_codes);

-- sequence for generating human-readable family IDs (starts at 1000 for professional appearance)
CREATE SEQUENCE IF NOT EXISTS configuration_family_id_seq START 1000 INCREMENT 1;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_one_active_per_snomed
...
CREATE INDEX IF NOT EXISTS idx_activations_active_lookup
ON activations (jurisdiction_id, snomed_code)
WHERE deactivated_at IS NULL;
...
CREATE TABLE IF NOT EXISTS configuration_labels (
