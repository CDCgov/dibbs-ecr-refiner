...
- `prune_container_guard_xpath`: pair with `prune_container_xpath` when an entry carries sibling containers that are shared, entry-scoped context rather than match candidates — they should survive alongside a matched sibling instead of being pruned as "non-matching." A container is only eligible for pruning when this relative xpath matches inside it. Used for Results, where the `organizer` carries a Specimen Collection Procedure (…4.415) and a Laboratory Result Status (…4.418) that apply to the whole battery; the guard (`.//observation[templateId=Result Observation V3]`) keeps those siblings while still pruning non-matching result observations. Keyed to the match target's templateId — not "any observation" — precisely so Lab Result Status (its own observation template) is treated as context, not a candidate.
