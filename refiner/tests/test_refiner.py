...
def test_health_check():
    actual_response = client.get("/message-refiner/healthcheck")
    assert actual_response.status_code == 200
    assert actual_response.json() == {"status": "OK"}


def test_openapi():
    actual_response = client.get("/message-refiner/api/openapi.json")
    assert actual_response.status_code == 200
...
def test_ecr_refiner():
    ...
    endpoint = "/message-refiner/api/v1/ecr"
    ...
    endpoint = f"/message-refiner/api/v1/ecr/?sections_to_include={sections_to_include}"
    actual_response = client.post(endpoint, content=content)
    assert actual_response.status_code == 200
...
async def test_ecr_refiner_conditions(mock_get):
    ...
    endpoint = (
        f"/message-refiner/api/v1/ecr/?conditions_to_include={conditions_to_include}"
    )
    actual_response = client.post(endpoint, content=content)
    assert actual_response.status_code == 200
    ...
    endpoint = f"/message-refiner/api/v1/ecr/?sections_to_include={sections_to_include}&conditions_to_include={conditions_to_include}"
    actual_response = client.post(endpoint, content=content)
    assert actual_response.status_code == 200
...
def test_ecr_refiner_zip():
    ...
    response = client.post(
        "message-refiner/api/v1/ecr/zip-upload?sections_to_include=blah blah blah",
        files={"file": ("test.zip", zip_bytes, "application/zip")},
    )
